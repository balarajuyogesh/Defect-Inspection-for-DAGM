...
class Model:
    ...
    def _build_net(self):        
        with tf.variable_scope(self._name):
            ...
            l1 = tf.layers.max_pooling2d(inputs=l1, pool_size=[2,2], padding="SAME", strides=2)               # (?,8,8,64)16            
            l1 = tf.layers.dropout(inputs=l1, rate=0.7, training=self.training)
            
            # Convolutional Layer #2 and Pooling Layer #3
            out = tf.layers.conv2d(inputs=l1, filters=128, kernel_size=[3,3],
                                   padding="SAME", activation=tf.nn.relu,
                                   kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                   kernel_regularizer=self.kernel_regularizer,
            ...
            out = tf.layers.dropout(inputs=out, rate=0.7, training=self.training)
            ...
            out = tf.reshape(out, [-1, 256*4*4])                              # 32-(?,4*4*128)  # 60-(?,8*8*128) 
            ...
            self.logits = tf.layers.dense(inputs=out, units=self._lable_size, 
                                          activation=None, 
                                          kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                          kernel_regularizer=self.kernel_regularizer,
